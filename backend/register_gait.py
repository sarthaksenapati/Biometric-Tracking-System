...
def register_gait(person_id: str):
    ...
    while len(sequences_collected) < TARGET_SEQUENCES:
        ...
        cv2.rectangle(display, (bar_x, bar_y),
                      (bar_x + bar_w, bar_y + bar_h_size), (60, 60, 60), -1)
        filled = int(bar_w * (len(sequences_collected) / TARGET_SEQUENCES))
        cv2.rectangle(display, (bar_x, bar_y),
                      (bar_x + filled, bar_y + bar_h_size), (0, 200, 100), -1)
        cv2.putText(display,
        ...
        if key == ord('s'):
            if len(frame_sequence) < MIN_FRAMES:
                print(f"  ⚠️  Not enough frames yet "
                      f"({len(frame_sequence)}/{MIN_FRAMES}) — keep walking")
                continue
            ...
            try:
                ...
                if hasattr(gait_model, 'get_embedding'):
                    ...
                    emb = gait_model.extract_gait_embedding(
                        frame_sequence[-MAX_FRAMES:]
                    )
                else:
                    print("  ❌ GaitModel has no get_embedding or "
                          "extract_gait_embedding method")
                    break

                if emb is not None:
                    emb = emb / (np.linalg.norm(emb) + 1e-8)
                    sequences_collected.append(emb)
                    print(f"  ✅ Sequence {len(sequences_collected)}/{TARGET_SEQUENCES} "
                    ...
                    if len(sequences_collected) < TARGET_SEQUENCES:
                        print(f"     ➡️  Walk {len(sequences_collected) + 1} — "
                              f"walk past again and press 's'\n")
                else:
